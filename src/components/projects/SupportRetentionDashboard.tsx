  ...
  return (
    <div className="space-y-8">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <MetricCard title="Overall Conversion" value={conversionRate} unit="%" status="warning" />
        <MetricCard title="CSAT Score (Dec)" value={currentSupport.csat_score.toFixed(1)} unit="/5" delta={0.1} deltaLabel="vs Nov" status={currentSupport.csat_score >= 4 ? "healthy" : "warning"} />
        <MetricCard title="Support Tickets (Dec)" value={currentSupport.support_tickets} delta={ticketDelta} deltaLabel="vs Nov" status={ticketDelta < 0 ? "healthy" : "warning"} />
        <MetricCard title="Revenue at Risk (Dec)" value={`£${(currentRevenue.revenue_at_risk_usd / 1000).toFixed(1)}k`} status="warning" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-card p-6 rounded-lg border border-border">
          <h3 className="text-lg font-semibold text-foreground mb-2">Customer Journey Funnel</h3>
          <p className="text-sm text-muted-foreground mb-6">User progression through key stages</p>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={cxJourneyFunnelData} layout="vertical">
                ...
                <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "0.5rem" }} formatter={(value: number) => [`${value} users`, "Count"]} />
        ...
        <div className="bg-card p-6 rounded-lg border border-border">
          <h3 className="text-lg font-semibold text-foreground mb-2">Support Operations Trend</h3>
          <p className="text-sm text-muted-foreground mb-6">Monthly tickets, resolution time, and CSAT</p>
          <div className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={cxSupportTrendMetrics}>
                ...
                <YAxis yAxisId="right" orientation="right" className="text-muted-foreground text-xs" domain={[3.5, 4.5]} />
                <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "0.5rem" }} />
                <Legend />
                <Bar yAxisId="left" dataKey="support_tickets" name="Support Tickets" fill="hsl(var(--primary))" radius={[4, 4, 0, 0]} />
                <Line yAxisId="right" type="monotone" dataKey="csat_score" name="CSAT Score" stroke="hsl(var(--status-healthy))" strokeWidth={2} dot={{ fill: "hsl(var(--status-healthy))", strokeWidth: 2, r: 3 }} />
              </ComposedChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="bg-card p-6 rounded-lg border border-border">
        <h3 className="text-lg font-semibold text-foreground mb-2">Revenue Impact Trend</h3>
        <p className="text-sm text-muted-foreground mb-6">Monthly revenue at risk vs retained revenue</p>
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <ComposedChart data={cxRevenueImpactTrends}>
              <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
              <XAxis dataKey="month" className="text-muted-foreground text-xs" />
              <YAxis className="text-muted-foreground text-xs" tickFormatter={(v) => `£${v / 1000}k`} />
              <Tooltip
                contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "0.5rem" }}
                formatter={(value: number) => [`£${(value / 1000).toFixed(1)}k`]}
              />
              <Legend />
              <Area type="monotone" dataKey="retained_revenue_usd" name="Retained Revenue" fill="hsl(var(--status-healthy) / 0.2)" stroke="hsl(var(--status-healthy))" strokeWidth={2} />
              <Line type="monotone" dataKey="revenue_at_risk_usd" name="Revenue at Risk" stroke="hsl(var(--status-risk))" strokeWidth={2} dot={{ fill: "hsl(var(--status-risk))", strokeWidth: 2, r: 3 }} />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-card p-6 rounded-lg border border-border">
        <h3 className="text-lg font-semibold text-foreground mb-2">Resolution Time Trend</h3>
        <p className="text-sm text-muted-foreground mb-6">Average hours to resolve support tickets</p>
        <div className="h-48">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={cxSupportTrendMetrics}>
              <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
              <XAxis dataKey="month" className="text-muted-foreground text-xs" />
              <YAxis className="text-muted-foreground text-xs" tickFormatter={(v) => `${v}h`} domain={[5, 8]} />
              ...
              <Line type="monotone" dataKey="avg_resolution_hours" stroke="hsl(var(--status-warning))" strokeWidth={2} dot={{ fill: "hsl(var(--status-warning))", strokeWidth: 2, r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-card p-6 rounded-lg border border-border">
        <h3 className="text-lg font-semibold text-foreground mb-2">Cohort Retention Curve (Nov 2024)</h3>
        <p className="text-sm text-muted-foreground mb-6">Retention decay over first 3 months</p>
        <div className="h-48">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={cxRetentionCohortData}>
              <CartesianGrid strokeDasharray="3 3" className="stroke-border" />
              <XAxis dataKey="month_number" className="text-muted-foreground text-xs" tickFormatter={(v) => `Month ${v}`} />
              <YAxis domain={[0, 100]} className="text-muted-foreground text-xs" tickFormatter={(v) => `${v}%`} />
              <Tooltip contentStyle={{ backgroundColor: "hsl(var(--card))", border: "1px solid hsl(var(--border))", borderRadius: "0.5rem" }} formatter={(value: number) => [`${value}%`, "Retention"]} labelFormatter={(label) => `Month ${label}`} />
              <Line type="monotone" dataKey="retention_pct" stroke="hsl(var(--primary))" strokeWidth={3} dot={{ fill: "hsl(var(--primary))", strokeWidth: 2, r: 5 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
